...
that protocol. The seed-1111 encoders, smoke gate, repaired MFON, and P4
Constant runs are complete; P4 Learned remains pending. No MOSEI test
result may be used to revise P4 or select an
allocation variant.
...
allocation direction, or corruption schedule. Server-side tests pass, the
seed-1111 acoustic and visual encoders are complete, and the two-epoch P4
Learned smoke satisfies its budget, reliability-gap, clean-task-path,
checkpoint-save, reload, and full-test gates. The formal seed-1111 repaired
MFON and matched P4 Constant runs have completed 25-epoch training and
checkpoint-reloaded testing. Constant obtains MAE 0.5406 and correlation
0.7735; the remaining task metrics are retained in the experiment ledger.
P4 Learned remains queued under the unchanged frozen configuration. No
two-epoch smoke metric is treated as performance evidence, and no comparative
MOSEI claim is included until the matched Learned cell completes. MOSEI test
outputs are reporting-only; any method revision would require a new untouched
confirmation source.
...
confirmation. Second, the confirmatory MOSEI comparison is incomplete.
...
Sixth, baseline loss is unavailable, preventing a complete loss comparison
with repaired MFON.
...
classification remains mixed. Because MOSI diagnostics participated in method
development, the frozen MOSEI comparison is required before making a
cross-dataset effectiveness claim. Acoustic length dependence, text-dominant
inference, and missing realistic stress tests further bound the scope. The
present evidence establishes the audit and budget-control mechanism;
empirical effectiveness remains provisional until confirmatory evaluation is
complete.
