...
that protocol. The seed-1111 encoders, smoke gate, and repaired MFON run
are complete; P4 Constant is the current formal run and P4 Learned remains
pending. No MOSEI test result may be used to revise P4 or select an
allocation variant.
...
MFON run has completed 25-epoch training and checkpoint-reloaded testing.
The matched P4 Constant run is currently in progress, and P4 Learned is
queued only after Constant completes. No two-epoch smoke metric is treated
as performance evidence, and no comparative MOSEI claim is included until
the matched comparison completes. From this point onward, MOSEI test outputs
are reporting-only; any method revision would require a new untouched
confirmation source.
