...
\item
  We report an exploratory three-seed CMU-MOSI study and a frozen
  three-seed CMU-MOSEI confirmation, then use Gaussian, timestep-dropout,
  contiguous-mask, temporal-shift, and modality-missing audits to distinguish
  in-family degradation detection from cross-corruption generalization.
\end{enumerate}
...
suite passes all 37 tests and Python syntax compilation. Coverage includes
...
\subsection{Three-seed confirmatory task results on
MOSEI}\label{mosei-confirmatory-results}
...
the corruption schedule. All three prespecified seeds of repaired MFON, P4
...
worsens MAE by 0.0004 and loss by 0.0010. The confirmation therefore supports
a favorable primary-endpoint direction over uniform allocation, not uniform
superiority to the base model.
...
& $0.8579\pm0.0081$ & $0.8576\pm0.0080$ & $0.5513\pm0.0075$
& $0.5347\pm0.0070$ & $0.5312\pm0.0053$ & $0.7746\pm0.0035$
& $0.495199\pm0.005712$ \\
P4 Constant & $0.8193\pm0.0204$ & $0.8228\pm0.0161$
& $0.8549\pm0.0040$ & $\mathbf{0.8536\pm0.0067}$
& $\mathbf{0.5556\pm0.0089}$ & $\mathbf{0.5383\pm0.0088}$
...
& $0.5543\pm0.0082$ & $0.5375\pm0.0078$
...
\caption{Frozen MOSEI task comparison under an equal auxiliary budget.
\textbf{a--c}, Mean MAE, correlation, and test loss for repaired MFON, P4
Constant, and P4 Learned; error bars denote sample standard deviation over
seeds 1111, 1112, and 1113 ($n=3$ seeds). \textbf{d}, Favourable-direction
difference between Learned and Constant across all reported endpoints; MAE
and loss are sign-reversed so that bars to the right consistently favour
Learned. The panel shows favourable regression directions but mixed
classification effects and does not constitute a significance test. Source
...
& Inf. (ms/batch) $\downarrow$ & Samples/s $\uparrow$
& Inf. peak (GiB) $\downarrow$ & F+B (ms/batch) $\downarrow$
& F+B peak (GiB) $\downarrow$ \\
\midrule
Repaired MFON & 130.765 & 0 & 585.14 & 1040.20 & 30.76 & 1.136
& 1112.09 & 4.791 \\
P4 Constant & 130.796 & 30,914 & 585.14 & 1009.84 & 31.69 & 1.136
& 1135.09 & 4.805 \\
P4 Learned & 130.796 & 30,914 & 585.14 & 1024.97 & 31.22 & 1.136
& 1120.03 & 4.805 \\
\bottomrule
\end{tabular}%
}
...
over P4 Constant on both the exploratory MOSI study and the frozen MOSEI
confirmation. This is consistent with sample assignment having a repeatable
but small effect under a fixed budget. MOSEI fine-grained metrics and Non0 F1
do not improve simultaneously, however, and Learned does not improve MAE or
loss over repaired MFON. The confirmation therefore supports the
primary-endpoint direction against the equal-budget Constant control, not
...
redesign, so MOSI remains exploratory rather than an untouched confirmation.
Second, although the frozen three-seed MOSEI confirmation is complete, three
seeds do not support statistical significance and primary endpoints do not
uniformly improve over repaired MFON. Third, reliability-proportional
allocation assumes that higher input reliability implies more trustworthy
auxiliary targets; final-schedule inverse/difficulty-aware and permutation
controls remain incomplete. Fourth, interventions operate on pre-extracted
...
Cross-corruption audits further show that Gaussian degradation detection does
not transfer automatically to timestep dropout, temporal shift, or complete
visual absence. A uniform microbenchmark finds that P4 adds approximately
...
text-dominant inference,
incomplete final-schedule actionability controls, and the lack of real-media
stress tests bound the scope. The evidence establishes a transparent and
falsifiable audit-and-budget-control mechanism, not a general quality
estimator or a validated robust-fusion model.
