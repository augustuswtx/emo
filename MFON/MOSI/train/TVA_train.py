...
def TVA_train_fusion(config, metrics, seed, train_data, valid_data):
    ...
    use_css = getattr(config.MOSI.downStream.TVAtrain, 'use_css', False)
    css_epoch = getattr(config.MOSI.downStream.TVAtrain, 'css_epoch', 20)
    css_min_ratio = getattr(config.MOSI.downStream.TVAtrain, 'css_min_ratio', 0.2)
