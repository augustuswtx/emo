...
The frozen P4 implementation has been ported to MOSEI without changing the original feature dimensions, learning rates, base auxiliary weights, allocation direction, or corruption schedule. Server-side tests pass, the seed-1111 acoustic and visual encoders are complete, and the two-epoch P4 Learned smoke satisfies its budget, reliability-gap, clean-task-path, checkpoint-save, reload, and full-test gates. The formal seed-1111 repaired MFON and matched P4 Constant runs have completed 25-epoch training and checkpoint-reloaded testing. Constant obtains MAE 0.5406 and correlation 0.7735; its remaining task metrics are retained in the experiment ledger. P4 Learned remains queued under the unchanged frozen configuration. No two-epoch smoke metric is treated as performance evidence, and no comparative MOSEI claim is made until the matched Learned cell completes. MOSEI test outputs are reporting-only: any method revision would require a new untouched confirmation source.
...
| Learned allocation differs meaningfully from uniform allocation | P4 Learned versus P4 Constant across three MOSI seeds | Exploratory descriptive support; confirmatory MOSEI comparison pending |
...
| The method generalizes across datasets | Frozen MOSEI comparison is in progress; SIMS not ported | Needs evidence |
