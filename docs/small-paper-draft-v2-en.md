...
> **Manuscript status:** evidence-grounded English major-revision draft v4, revised on 15 September 2026. Completed results are preserved; C1--C5 experiments remain explicitly TBD.
>
...
4. We report an exploratory three-seed CMU-MOSI study and a frozen three-seed CMU-MOSEI replication, then use Gaussian, timestep-dropout, contiguous-mask, temporal-shift, and modality-missing audits to distinguish in-family degradation detection from cross-corruption generalization. We expose cross-sample score comparability and target fidelity as unresolved assumptions rather than treating high Gaussian AUROC as their validation.
