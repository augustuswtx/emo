...
> **2026-08-16 审稿后协议调整：** MOSI 测试集可靠性诊断曾影响视觉头保留与音频头重设计，因此 MOSI 统一定位为方法开发与探索性证据。P4 已在 MOSEI 正式比较前冻结；MOSEI 从现在起承担确认性验证，测试输出只用于报告，不得用于修改方法、超参数、分配方向或决定是否选择性扩展随机种子。seed 1111 P4 Constant 已完成训练、预算检查、检查点保存与同配置重载测试；下一项仅为冻结配置的 seed 1111 P4 Learned。
...
| 3 | MOSEI seed 1111 P4 Learned | next / queued | 冻结配置下完成 25 轮与同配置重载测试 | 不依据 Constant test 改配置 |
...
## MOSEI 主次指标冻结（2026-08-16，P4 Constant 正式测试结果产生前）

- **主要终点：** MAE（越低越好）与 Pearson Corr（越高越好）。选择依据是训练任务本身为情感回归，而不是当前已知结果方向。
- **次要终点：** Has0/Non0 Acc-2 与 F1。
- **诊断终点：** Acc-5、Acc-7、测试 Loss、可靠性 Spearman/AUROC、长度/能量混杂、权重离散度。
- **报告单位：** 每个 seed 的原始结果、Learned−Constant 配对差值、三 seed 均值与样本标准差；全部指标完整报告，不按方向筛选。
- **解释边界：** 三 seed 只提供描述性重复证据。bootstrap 置信区间用于显示不确定性，不将其包装为充分功效的显著性检验。
- **方向假设：** 当前 Learned 假设高可靠性样本的辅助目标更可信；最终日程的 Inverse/Difficulty-aware 控制用于检验该假设。若 Inverse 更优，应修改机制解释，而不是删除该控制。
